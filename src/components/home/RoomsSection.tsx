        ...
        <div className="grid md:grid-cols-2 gap-8 lg:gap-12">
          {rooms.map((room, index) => (
            <ScrollReveal key={room.id} delay={index * 100}>
              <Link
                to={`/rooms/${room.id}`}
                className="group flex flex-col h-full bg-card rounded-lg overflow-hidden shadow-card hover:shadow-elevated transition-all duration-500"
              >
                {/* Image */}
                ...
                <div className="p-6 lg:p-8 flex flex-col flex-1">
                  ...
                  <span className="inline-flex items-center text-primary text-sm font-medium group-hover:gap-3 gap-2 transition-all mt-auto">
                    {t("rooms.view")}
